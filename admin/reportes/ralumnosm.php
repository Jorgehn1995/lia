...
function calcularedadesm($idcole,$genero){
  $r=0;
  ...
  $sentencia = "SELECT * FROM `alumnos` WHERE idcole='$idcole' AND genero='$genero'";
  ...
  if ($query2) {
    while ($a=mysqli_fetch_array($query2)) {
      ...
      $cn=mysqli_query($conexion,$sql);
      $sentencia = "SELECT * FROM `alumnos` WHERE idcole='$idcole' AND idgrado='$idgrado' AND genero='$genero'";
      $resultado = mysqli_query($conexion,$sentencia);
      while ($g=mysqli_fetch_array($resultado)) {
        $nacimiento=$g['na']."-".$g['nm']."-".$g['nd'];
        $actual=date("Y-m-d");
...
if ($query) {
	while ($a=mysqli_fetch_array($query)) {
		...
		$sql2="SELECT * FROM `alumnos` WHERE idcole='$idcole' AND idgrado='$idgrado' AND genero='$genero'";
		$query2=mysqli_query($conexion,$sql2);
	...
	//Cantidad de hombres y mujeres
	$sql2="SELECT * FROM `alumnos` WHERE idcole='$idcole' AND genero='$genero'";
	$query2=mysqli_query($conexion,$sql2);
	$total=$query2->num_rows;
