...
if ($resultado) {
  while( $data = mysqli_fetch_array($resultado)){
    ...
    $asesor=0;
    $sqlasesor="SELECT * FROM `modelocuadro` WHERE idmodelo='$idactividad' LIMIT 1";
    $queryasesor=mysqli_query($conexion,$sqlasesor);
    while ($d=mysqli_fetch_array($queryasesor)) {
      $asesor=$d['asesor'];
    }
    $nombre=$data['nombre'];
    if ($asesor==0) {
      ...
      $sqlnotas="SELECT idnota, asignado, obtenido FROM `notasasesores` WHERE idalumno='$idalumno' AND idbloque='$bloque' AND idmodelo='$idactividad'";
      $conect=mysqli_query($conexion,$sqlnotas);
      if ($conect) {
        $filas=$conect->num_rows;
        if ($filas==0) {
        ...
        while ($a=mysqli_fetch_array($conect)) {
          echo '<div class="inbox-item ">
            ...
            <p class="inbox-item-date"></p>
            <div class="form-group">
              '.inputtotal(cero($a['obtenido']),$idactividad,$a['idnota']).'
            </div>
          </div>';
        }
      }else {
