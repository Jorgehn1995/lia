      ...
      <ul class="navigation-menu">
        ...
        for ($i=$bloqueencurso; $i ==$bloqueencurso ; $i++) {
          ...
          echo '<li class="has-submenu">
            <a href="#"><i class="  mdi mdi-numeric-'.$i.'-box-multiple-outline  "></i>Cuadros Bloque '.$bq[$i-1].'</a>
            <ul class="submenu">';
         ...
         for ($i=$bloqueencurso; $i ==$bloqueencurso ; $i++) {
           $bq = array("I" , "II","III","IV" );
           echo '<li class="has-submenu">
             <a href="#"><i class="  mdi mdi-numeric-'.$i.'-box-multiple-outline  "></i>Actividades Bloque '.$bq[$i-1].'</a>
             <ul class="submenu">';
           require '../../conexion/conexion.php';
           $sql="SELECT materias.idnombremateria, materias.idgrado, materias.seccion, nombrematerias.corto, nombrematerias.nombre FROM `materias` INNER JOIN `nombrematerias` ON materias.idnombremateria=nombrematerias.idnombremateria  WHERE materias.idprofesor='$idusuario'  GROUP BY idnombremateria";
           //echo $usuario;
           ...
           if ($con) {
             while ($a=mysqli_fetch_array($con)) {
               echo '
                   <li><a href="../notasxactividad/?idmateria='.$a['idnombremateria'].'&bloque='.$i.'&materia='.$a['nombre'].'">'.$a['corto'].'</a></li>';
             }
           }else {
             echo "Error ".mysqli_errno($conexion).": ".mysqli_error($conexion);
           }
          ...
          ?>
         <li class="has-submenu">
           <a href="../main/close.php"><i class="  ti-layers-alt  "></i>Bloques</a>
         </li>
        ...
        <li class="has-submenu">
          <a href="../main/close.php"><i class="  ti-cloud "></i>Nube</a>
        </li>
        <li class="has-submenu">
